...
test.describe('Library Agent', () => {
  ...
  test('shows Library Agent card', async ({ page }) => {
    await expect(page.locator('.card-title:has-text("Library Agent")')).toBeVisible({ timeout: 5000 });
  });

  test('shows Run Agent button on card', async ({ page }) => {
    const libraryCard = page.locator('.card-title:has-text("Library Agent")').locator('..').locator('..');
    await expect(libraryCard.locator('.btn:has-text("Run Agent")')).toBeVisible({ timeout: 5000 });
  });

  test('opens agent modal on Run Agent click', async ({ page }) => {
    const libraryCard = page.locator('.card-title:has-text("Library Agent")').locator('..').locator('..');
    await libraryCard.locator('.btn:has-text("Run Agent")').click();
    const modal = page.locator('#agentModal');
    await expect(modal).toBeVisible({ timeout: 5000 });
    await expect(modal.locator('.modal-title')).toContainText('Library Agent');
  });
  ...
  test('shows overdue days options in modal', async ({ page }) => {
    await openModal(page);
    await expect(page.locator('#agentConfigFields select.agent-param')).toBeVisible();
    const select = page.locator('#agentConfigFields select.agent-param');
    await expect(select.locator('option[value="1"]')).toBeVisible();
    await expect(select.locator('option[value="7"]')).toBeVisible();
    await expect(select.locator('option[value="14"]')).toBeVisible();
    await expect(select.locator('option[value="30"]')).toBeVisible();
  });
  ...
  test('previews overdue books on button click', async ({ page }) => {
    await openModal(page);
  ...
  test('navigates back after execution', async ({ page }) => {
    ...
    await page.goto('/admin/agents', { timeout: 30000 });
    await page.locator('.card-title:has-text("Library Agent")').waitFor({ timeout: 10000 });
    await expect(page.locator('.card-title:has-text("Library Agent")')).toBeVisible();
  });

  test('shows book details after preview', async ({ page }) => {
    await openModal(page);
    await page.locator('#agentPreviewBtn').click();
    await expect(page.locator('#agentStepConfirm')).toBeVisible({ timeout: 15000 });
  ...
  test('allows days selection', async ({ page }) => {
    await openModal(page);
    const select = page.locator('#agentConfigFields select.agent-param');
