...
test.describe('Attendance Agent', () => {
  ...
  test('shows Attendance Agent card', async ({ page }) => {
    await expect(page.locator('.card-title:has-text("Attendance Agent")')).toBeVisible({ timeout: 5000 });
  });

  test('shows Run Agent button on card', async ({ page }) => {
    const attendanceCard = page.locator('.card-title:has-text("Attendance Agent")').locator('..').locator('..');
    await expect(attendanceCard.locator('.btn:has-text("Run Agent")')).toBeVisible({ timeout: 5000 });
  });

  test('opens agent modal on Run Agent click', async ({ page }) => {
    const attendanceCard = page.locator('.card-title:has-text("Attendance Agent")').locator('..').locator('..');
    await attendanceCard.locator('.btn:has-text("Run Agent")').click();
    const modal = page.locator('#agentModal');
    await expect(modal).toBeVisible({ timeout: 5000 });
    await expect(modal.locator('.modal-title')).toContainText('Attendance Agent');
  });
  ...
  test('shows date input in modal', async ({ page }) => {
    await openModal(page);
    const dateInput = page.locator('#agentConfigFields input[type="date"]');
    await expect(dateInput).toBeVisible();
    await expect(dateInput).toHaveValue(new Date().toISOString().split('T')[0]);
  });
  ...
  test('previews attendance data on button click', async ({ page }) => {
    await openModal(page);
  ...
  test('navigates back after execution', async ({ page }) => {
    ...
    await page.goto('/admin/agents', { timeout: 30000 });
    await page.locator('.card-title:has-text("Attendance Agent")').waitFor({ timeout: 10000 });
    await expect(page.locator('.card-title:has-text("Attendance Agent")')).toBeVisible();
  });
  ...
  test('allows date selection', async ({ page }) => {
    await openModal(page);
    const dateInput = page.locator('#agentConfigFields input[type="date"]');
    await dateInput.fill('2026-06-23');
    await expect(dateInput).toHaveValue('2026-06-23');
  });
