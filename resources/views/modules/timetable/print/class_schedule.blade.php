...
@section('content')
    ...
    <div class="card">
        <div class="card-header d-flex align-items-center justify-content-between">
            <div>
                <h3 class="fw-semibold card-title mb-0 d-inline-block">{{ $classSection->schoolClass->name }} - {{ $classSection->section->name }}</h3>
        ...
        <div class="card-body">
            ...
            <div class="d-none d-print-block text-center mb-4 pb-3 border-bottom">
                ...
                <h4 class="mb-2 text-muted">Class Timetable ({{ $academicYear->name }})</h4>
                <h3 class="mb-0 fw-semibold">{{ $classSection->schoolClass->name }} - {{ $classSection->section->name }}</h3>
            </div>
            @if($schedule->isEmpty())
                        ...
                        @foreach($activeDays as $day)
                            <tr>
                                ...
                                @foreach($periods as $period)
                                    ...
                                    <td>
                                        @if($slot)
                                            <div class="fw-semibold text-dark mb-1">{{ $slot->subject?->name }}</div>
                                            <div class="small text-muted">{{ $slot->teacher?->full_name }}</div>
                                            @if($slot->room)
            ...
            <div class="d-none d-print-block mt-5 pt-5">
                <div class="row text-center mt-5">
                    <div class="col-4">
                        ...
                        <div class="fw-semibold mt-2">Class Teacher</div>
                    </div>
                    <div class="col-4">
                        <hr class="w-50 mx-auto border-dark" style="border-width: 2px;">
                        <div class="fw-semibold mt-2">Coordinator</div>
                    </div>
                    <div class="col-4">
                        <hr class="w-50 mx-auto border-dark" style="border-width: 2px;">
