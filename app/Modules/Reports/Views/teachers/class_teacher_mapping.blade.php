...
@section("title", "Class Teacher Mapping Report")
@section("page-title", "Class Teacher Mapping Report")
...
$(async function() {
    ...
    let table = $('#report-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "{{ route('reports.teachers.class_teacher_mapping') }}",
            data: function (d) {
        ...
        columns: [
            ...
            { 
                data: 'class_teacher_sections', 
                name: 'class_teacher_sections',
                render: function(data) {
                    if (!data || data.length === 0) return 'None';
                    return data.map(function(s) { return s.school_class.name + ' - ' + s.section.name; }).join(', ');
                }
            }
        ]
    });
    ...
    $('.export-btn').on('click', function(e) {
        ...
        let url = "";
        if (type === 'pdf') {
            url = "{{ route('reports.teachers.export.pdf', ['type' => 'class_teacher_mapping']) }}";
        } else if (type === 'excel') {
            url = "{{ route('reports.teachers.export.excel', ['type' => 'class_teacher_mapping']) }}";
        } else if (type === 'print') {
            url = "{{ route('reports.teachers.print', ['type' => 'class_teacher_mapping']) }}";
